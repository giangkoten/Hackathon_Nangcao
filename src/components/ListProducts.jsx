...
import FormItemBuy from "./FormItemBuy";
...
function ListProducts() {
  ...
  const handleInCrease = (item) => {
    const dataIncrease = data.map((itemBuy) => {
      if (itemBuy.id === item.id) {
        return { ...itemBuy, numberBuy: itemBuy.numberBuy + 1 };
      }
      return itemBuy;
    });
    setData(dataIncrease);
  };

  const handleDecrease = (item) => {
    const dataDecrease = data.map((itemBuy) => {
      if (itemBuy.id === item.id && itemBuy.numberBuy > 0) {
        return { ...itemBuy, numberBuy: itemBuy.numberBuy - 1 };
      }
      return itemBuy;
    });
    setData(dataDecrease);
  };

  const deleteContrustor = (id) => {
    const newUpdateItem = data.filter((item) => item.id !== id);
    setData(newUpdateItem);
  };
  ...
  return (
    <div>
      <h1>List Products</h1>
      <h1 onClick={handleToggle}>Cart</h1>
      {isCartVisible == true ? (
        ...
        <FormItemBuy
          dataProps={data}
          deleteContrustor={deleteContrustor}
        ></FormItemBuy>
      )}
    </div>
  );
